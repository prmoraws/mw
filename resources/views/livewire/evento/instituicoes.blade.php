            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                            ...
                            <form wire:submit.prevent="store">
                                ...
                                <div class="flex justify-end gap-2 mt-6">
                                    ...
                                    <button type="submit"
                                        ...
                                        {{ $instituicao_id ? 'Atualizar' : 'Criar' }}
                                    </button>
                                </div>
                            </form>
                        </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-700 dark:text-gray-300"><strong>Localização:</strong> <a
                                                href="{{ $selectedInstituicao->localização }}" target="_blank"
                    ...
                    <div wire:key="delete-modal" x-data="{ open: {{ $confirmDeleteId ? 'true' : 'false' }} }" x-show="open"
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md mx-4 p-6">
                            ...
                            <p class="text-gray-600 dark:text-gray-300 mb-6">Tem certeza que deseja apagar esta
                                instituição? Esta ação não pode ser desfeita.</p>
                ...
                @if (!empty($results))
                        ...
                        <table class="w-full table-auto border-collapse">
                            <thead>
                                <tr class="bg-gray-100 dark:bg-gray-700">
                                    ...
                                    <th scope="col" wire:click="sortBy('convidados')"
                                        class="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200 cursor-pointer">
                                        Convidados @if ($sortField === 'convidados')
                                            <span>{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                                        @endif
                                    </th>
                                    <th scope="col" wire:click="sortBy('onibus')"
                                        class="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200 cursor-pointer">
                                        Ônibus @if ($sortField === 'onibus')
                                            <span>{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                                        @endif
                                    </th>
                            ...
                            <tbody>
                                @foreach ($results as $index => $instituicao)
                                    <tr wire:key="table-row-{{ $instituicao->id }}-{{ $index }}"
                                        ...
                                        <td class="border px-4 py-3 text-sm text-gray-700 dark:text-gray-200 truncate max-w-[150px]"
                                            title="{{ $instituicao->convidados }}">{{ $instituicao->convidados }}
                                        </td>
                                        <td class="border px-4 py-3 text-sm text-gray-700 dark:text-gray-200 truncate max-w-[150px]"
                    ...
                    <div class="md:hidden space-y-4">
                        @foreach ($results as $index => $instituicao)
                            <div wire:key="card-{{ $instituicao->id }}-{{ $index }}"
                                ...
                                <div class="flex justify-between items-center">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600 dark:text-gray-400">Ônibus:
                                            {{ $instituicao->onibus }}</p>
                ...
                @else
                    <p class="text-gray-500 dark:text-gray-400 text-sm mt-4">Nenhuma instituição encontrada. Use o
                        campo acima para pesquisar ou crie uma nova.</p>
                @endif
            </div>
        </div>
    </div>
