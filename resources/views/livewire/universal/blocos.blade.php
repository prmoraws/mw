    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                ...
                @if ($isOpen)
                    <div wire:key="create-edit-modal" x-data="{ open: {{ $isOpen ? 'true' : 'false' }} }" 
                        x-show="open" x-on:keydown.escape.window="open && $wire.closeModal()"
                ...
                @if ($confirmDeleteId)
                    <div wire:key="delete-modal" x-data="{ open: {{ $confirmDeleteId ? 'true' : 'false' }} }" 
                        x-show="open" x-on:keydown.escape.window="open && $wire.set('confirmDeleteId', null)"
                        x-transition:enter="transition ease-out duration-300"
                        x-transition:enter-start="opacity-0 transform scale-95"
                        x-transition:enter-end="opacity-100 transform scale-100"
                        x-transition:leave="transition ease-in duration-200"
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md mx-4 p-6">
                            ...
                            <p class="text-gray-600 dark:text-gray-300 mb-6">Tem certeza que deseja apagar este bloco? Esta ação não pode ser desfeita.</p>
                            <div class="flex justify-end gap-2">
                ...
                @if (!empty($results))
                        ...
                        <table class="w-full table-auto border-collapse">
                            <thead>
                                <tr class="bg-gray-100 dark:bg-gray-700">
                                    <th scope="col" class="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200">No.</th>
                            ...
                            <tbody>
                                @foreach ($results as $index => $bloco)
                                    <tr wire:key="table-row-{{ $bloco->id }}-{{ $index }}"
                                        class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }} dark:{{ $index % 2 == 0 ? 'bg-gray-800' : 'bg-gray-900' }}">
                                        <td class="border px-4 py-3 text-sm text-gray-700 dark:text-gray-200">{{ $bloco->id }}</td>
                                        <td class="border px-4 py-3 text-sm text-gray-700 dark:text-gray-200">{{ $bloco->nome }}</td>
                                        <td class="border px-4 py-3 flex gap-2">
                                            ...
                                            <button wire:click="teste({{ $bloco->id }})"
                                                class="bg-green-500 hover:bg-green-600 text-white text-sm py-1 px-2 rounded flex items-center gap-1"
                    ...
                    <div class="md:hidden space-y-4">
                        @foreach ($results as $index => $bloco)
                            <div wire:key="card-{{ $bloco->id }}-{{ $index }}"
                                class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm">
                                <div class="flex justify-between items-center">
                                    <div>
                                        <p class="text-sm font-medium text-gray-900 dark:text-gray-100">No. {{ $bloco->id }}</p>
                                        <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $bloco->nome }}</p>
                                    </div>
                ...
                @else
                    <p class="text-gray-500 dark:text-gray-400 text-sm mt-4">Nenhum bloco encontrado. Use o campo acima para pesquisar.</p>
                @endif
            </div>
        </div>
    </div>
